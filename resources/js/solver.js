...
document.addEventListener('mousedown', function(event) {
	if (event.target.id.startsWith('aspect_image_')) {
		...
		document.body.style.cursor = "url('./resources/images/aspects/" + keyAtIndex(aspect_flavor_dict, event.target.id.substring(13)) + '.png' + "') 15 15, pointer";
		holding_aspect = true;
...
function draw_hexagon(x, y, side_len, lineWidth = 1, lineColor = '#000000', fillColor = '#bfbfbf', pointing_up = false) {
...
function toggle_availability(id) {
	...
	if (found) {
		...
		document.getElementById('aspect_image_' + id).style.filter = 'brightness(10%)';
...
function set_default_aspect_set() {
	...
	for (let i = 0; i < keylen(all_aspect_table); i++) {
		if (!default_aspects.includes(keyAtIndex(all_aspect_table, i))) {
			...
			document.getElementById('aspect_image_' + i).style.filter = 'brightness(10%)';
...
function search(box) {
	...
	for (let i = 0; i < keylen(all_aspect_table); i++) {
		if (keyAtIndex(all_aspect_table, i).startsWith(box.value.toLowerCase())) {
			results_beginning.push(keyAtIndex(all_aspect_table, i));
		} else if (keyAtIndex(all_aspect_table, i).includes(box.value.toLowerCase())) {
			results_anywhere.push(keyAtIndex(all_aspect_table, i));
		}
	}
	//console.log(results_beginning);
	//console.log(results_anywhere);
	let aspect_selector_gui = document.getElementById('aspect_selector_gui');
	aspect_flavor_dict = {};
	for (let i = 0; i < results_beginning.length; i++) {
		aspect_flavor_dict[results_beginning[i]] = all_aspect_flavor_dict[results_beginning[i]];
	}
	for (let i = 0; i < results_anywhere.length; i++) {
		aspect_flavor_dict[results_anywhere[i]] = all_aspect_flavor_dict[results_anywhere[i]];
	}
	load_aspect_selector(false);
}
...
for (let i = 0; i < keylen(aspect_table); i++) {
	...
	temp_img.src = './resources/images/aspects/' + keyAtIndex(aspect_table, i) + '.png';
...
function tile_neighbors(x, y, grid, all = false) {
	...
	if (x == Math.floor(grid.length / 2)) {
		...
		for (let i = 0; i < left_coords.length; i++) {
			if (!cell_exists(grid, left_coords[i][0], left_coords[i][1]) || grid[left_coords[i][0]][left_coords[i][1]].aspect == 'none' || !((all_aspect_table[grid[x][y].aspect].includes(grid[left_coords[i][0]][left_coords[i][1]].aspect) || all_aspect_table[grid[left_coords[i][0]][left_coords[i][1]].aspect].includes(grid[x][y].aspect)) || all)) {
...
function solve_wrapper() {
	let wait_text = document.getElementById('wait_solving');
	wait_text.innerHTML = 'Please wait, solving...';
	let br_node = document.createElement('br');
	br_node.id = 'break';
	wait_text.after(br_node);
	setTimeout(function(){ solve(); }, 10);
}
...
function solve() {
	...
	document.getElementById('wait_solving').innerHTML = '';
	document.getElementById('break').remove()
}

function reset() {
	grid = draw_initial_grid(parseInt(document.getElementById('grid_size_input').value));
	current_state.grid = grid;
	draw_grid(grid);
	depth = 0;
	state_history = [];
